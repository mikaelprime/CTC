...
class SchedulesPage(CrudPage):
    def __init__(self, parent=None):
        ...
        super().__init__(
            ...
            columns=columns,
            fetch_fn=_fetch,
            # El cajero consulta los turnos; crearlos o cambiarlos es del admin.
            create_spec=create_spec if api.is_admin() else None,
            create_fn=_create,
            create_label="Nuevo turno",
            delete_fn=_delete if api.is_admin() else None,
            edit_spec=edit_spec if api.is_admin() else None,
            update_fn=_update,
